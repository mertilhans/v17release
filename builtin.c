...
int built_cd(t_parser *cmd)
{
    ...
    if (cmd->argv[1] == NULL) 
    {
        pwd = get_env_value(g_env_list, "HOME");
        if (!pwd)
        {
    ...
    // Update PWD and OLDPWD environment variables // gerek var buna bir önceki pwd yi saklayan değişken :D
    set_env_value(&g_env_list, "OLDPWD", oldpwd);
    
    if (getcwd(cwd, sizeof(cwd)) != NULL)
        set_env_value(&g_env_list, "PWD", cwd);
    ...
    while (cmd->argv[i])
    {
        ...
        if (eq_pos) // Variable assignment: VAR=value
        {
            *eq_pos = '\0'; // Split key and value
            set_env_value(&g_env_list, arg, eq_pos + 1);
            *eq_pos = '='; // Restore original string
        }
        else // Just export existing variable or create empty one
        {
            t_env *existing = find_env(g_env_list, arg);
            if (!existing)
                set_env_value(&g_env_list, arg, "");
        }
        i++;
    }
    return 0;
...
int builtin_unset(t_parser *cmd)
{
    ...
    while (cmd->argv[i])
    {
        ...
        unset_env_value(&g_env_list, var_name);
        i++;
    }
    return 0;
}
